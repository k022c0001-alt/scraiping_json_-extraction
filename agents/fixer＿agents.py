...
設計メモ:
    ...
    - 無限ループを防止するため、呼び出し側（ワークフロー）が管理する「修正ループの上限（max_retries）」
      と「現在の試行回数（current_retry）」を感知し、上限が近づくにつれて警告プロンプトを動的に強める。
"""
...
logger = logging.getLogger("OFA.FixerAgent")
...
class FixerAgent:
    ...
    def __init__(self, model_name: str = "gemini-2.5-pro"):
        ...
        if not api_key:
            logger.warning("[FixerAgent] GEMINI_API_KEY が設定されていません。")
    ...
    async def fix_artifact(
        ...
        """
        logger.info(
            "[FixerAgent] 修正処理を開始します。ループ状況: (%d / %d)", 
        ...
        if current_retry >= max_retries:
            ...
            必要であれば、実装のアプローチや設計をドラスティックに根本から見直し、視野を広く持って修正してください。
            """
        else:
            # まだ回数に余裕がある場合
            urgency_instruction = f"""
        ...
        try:
            ...
            response = await self.model.generate_content_async(
                ...
                generation_config=genai.GenerationConfig(
                    temperature=0.2,  # 修正の正確性と安定性のバランスを考慮
                )
            )

            logger.info("[FixerAgent] 修正版の出力を生成しました。")
            return response.text

        except Exception as e:
            logger.error("[FixerAgent] 修正処理中にエラーが発生しました: %s", e)
